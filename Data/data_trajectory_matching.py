...
class ReplacedDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        if i in self.poison_set:
            # allow poisoner deterministic behaviour if supported
            if hasattr(self.poisoner, "seed"):
                try:
                    self.poisoner.seed(i + self.seed)
                except TypeError:
                    # some seed implementations may not accept this; ignore if so
                    pass
            return self.poisoner(self.dataset[i])  # poisoner expects (x,y)
        else:
...
def get_matching_datasets(
    ...
    mtt_dataset = MTTDataset(train_poisoned, mtt_distill_dataset, poison_inds, train_transform, n_classes)
